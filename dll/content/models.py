...
class Content(ModelMeta, RulesModelMixin, PublisherModel, PolymorphicModel):
    ...
    co_authors = models.ManyToManyField(
        ...
        verbose_name=_("Kollaborateure"),
        blank=True,
    )
    image = FilerImageField(
        on_delete=models.SET_NULL, null=True, verbose_name=_("Anzeigebild"), blank=True
    )
    teaser = models.TextField(
        max_length=140, verbose_name=_("Teaser"), null=True, blank=True
    )
    learning_goals = ArrayField(
        models.CharField(max_length=850),
        verbose_name=_("Lernziele"),
        default=list,
        null=True,
        blank=True,
    )
    related_content = models.ManyToManyField(
        "self",
        verbose_name=_("Verwandte Tools/Trends/Unterrichtsbausteine"),
        blank=True,
    )
    ...
    additional_info = models.TextField(
        _("Hinweise/Anmerkungen/Hintergrund"), blank=True, null=True
    )
    competences = models.ManyToManyField(
        "Competence", verbose_name=_("Kompetenzen"), blank=True
    )
    sub_competences = models.ManyToManyField(
        "SubCompetence", verbose_name=_("Subkompetenzen"), blank=True
    )
    ...
    ex_authors = models.CharField(
        _("Ex-Autoren"), max_length=800, null=True, blank=True
    )
    ...
    def review(self):
        try:
            ...
            logger.error(
                "Multiple active reviews returned for Content with pk {}. "
                "Setting the latest one as active".format(self.pk)
            )
            review_pks = (
                self.reviews.filter(is_active=True)
                .order_by("-modified")
    ...
    def assign_reviewer(self, by_user: DllUser, to_be_assigned_user: DllUser):
        ...
        if (
            self.review.status == Review.ACCEPTED
            or self.review.status == Review.DECLINED
        ):
        ...
        if by_user != to_be_assigned_user:
            if by_user.has_perm("content.assign_reviewer"):
                ...
                raise AssertionError(
                    "User #{} is not allowed to assign reviewer.".format(by_user.id)
                )
        else:
            if by_user.has_perm("content.claim_review", self.review):
                ...
                raise AssertionError(
                    "User #{} is not allowed to claim review".format(by_user.id)
                )
    ...
    def unassign_reviewer(self, by_user: DllUser, to_be_unassigned_user: DllUser):
        ...
        if by_user != to_be_unassigned_user:
            if by_user.has_perm("content.assign_reviewer"):
                ...
                raise AssertionError(
                    "User #{} is not allowed to unassign reviewer.".format(by_user.id)
                )
        else:
            if by_user.has_perm("content.claim_review", self.review):
                review = self.review
                ...
                raise AssertionError(
                    "User #{} is not allowed to unclaim review".format(by_user.id)
                )
...
class TeachingModule(Content):
    ...
    subject_of_tuition = models.TextField(
        verbose_name=_("Unterichtsgegenstand"), max_length=2000, null=True, blank=True
    )
    educational_plan_reference = models.TextField(
        _("Bildungsplanbezug"), null=True, blank=True
    )
    school_class = IntegerRangeField(
        verbose_name=_("Jahrgangsstufe"), null=True, blank=True
    )
    # estimated time e.g. Doppelstunde,unterrichtsbegleitend
    estimated_time = models.CharField(
        max_length=300, verbose_name=_("Zeitumfang"), null=True, blank=True
    )
    equipment = ArrayField(
        models.CharField(max_length=350),
        verbose_name=_("Ausstattung"),
        default=list,
        null=True,
        blank=True,
    )
    state = models.CharField(
        _("Bundesland"), max_length=22, choices=GERMAN_STATES, null=True, blank=True
    )
    differentiating_attribute = models.TextField(
        _("Differenzierung"), null=True, blank=True
    )
    expertise = ArrayField(
        models.CharField(max_length=1500),
        verbose_name=_("Fachkompetenzen"),
        default=list,
        null=True,
        blank=True,
    )
    subjects = models.ManyToManyField(
        "Subject", verbose_name=_("Unterrichtsfach"), blank=True
    )
    school_types = models.ManyToManyField(
        "SchoolType", verbose_name=_("Schulform"), blank=True
    )
...
class Tool(Content):
    ...
    PRIVACY_CHOICES = (
        ...
        (
            2,
            _(
                "Personenbezogene Daten wie z.B. Logins werden geschützt auf dem Server abgelegt. "
        ...
        (
            3,
            _(
                "Personenbezogene Daten werden erhoben. Dritte haben Zugriff auf diese Daten. "
                "Es greift die EU-Datenschutz-Grundverordnung."
            ),
        ),
        (
    ...
    operating_systems = models.ManyToManyField(
        "OperatingSystem", verbose_name=_("Betriebssystem"), blank=True
    )
    applications = models.ManyToManyField(
        "ToolApplication", verbose_name=_("Anwendung"), blank=True
    )
    status = models.CharField(
        _("Status"), max_length=7, choices=STATUS_CHOICES, default=None, null=True
    )
    requires_registration = models.BooleanField(null=True, blank=False)
    usk = models.CharField(
        _("Altersfreigabe"), max_length=5, choices=USK_CHOICES, null=True, blank=True
    )
    pro = ArrayField(
    ...
    contra = ArrayField(
        models.CharField(max_length=300),
        verbose_name=_("Kontra"),
        default=list,
        null=True,
        blank=True,
    )
    privacy = models.IntegerField(
        _("Datenschutz"), choices=PRIVACY_CHOICES, null=True, blank=True
    )
    description = models.TextField(_("Beschreibung"), null=True, blank=True)
...
class Trend(Content):
    ...
    language = models.CharField(
        _("Sprache"), max_length=10, choices=LANGUAGE_CHOICHES, blank=True, null=True
    )
    licence = models.IntegerField(
        _("Lizenz"), choices=LICENCE_CHOICES, blank=True, null=True
    )
    category = models.IntegerField(
        _("Kategorie"), choices=CATEGORY_CHOICES, blank=True, null=True
    )
    ...
    publisher = ArrayField(
        models.CharField(max_length=300),
        verbose_name=_("Herausgeber"),
        default=list,
        null=True,
        blank=True,
    )
    publisher_date = models.DateField(
        _("Datum der Veröffentlichung"), blank=True, null=True
    )
    central_contents = models.TextField(_("Zentrale Inhalte"), blank=True, null=True)
    citation_info = models.CharField(
        _("Zitierhinweis"), max_length=500, blank=True, null=True
    )
...
class HelpText(TimeStampedModel):
    content_type = models.OneToOneField(
        ContentType, on_delete=models.CASCADE, related_name="help_text"
    )
    ...
    def get_help_text_fields_for_content_type(self):
        ...
        for field in fields:
            ...
            if not name.split(".")[-1].strip(">") in EXCLUDED_FIELDS:
                choices.append(
                    (str(field), str(getattr(field, "verbose_name", field.name)))
                )
    ...
    def __str__(self):
        return (
            _("Hilfetext für ")
            + self.content_type.model_class()._meta.verbose_name_plural.title()
        )


class HelpTextField(TimeStampedModel):
    name = models.CharField(max_length=100)
    help_text = models.ForeignKey(
        HelpText, on_delete=models.CASCADE, related_name="help_text_fields"
    )
...
class Review(TimeStampedModel):
    ...
    def send_review_accepted_mail(self):
        ...
        cc_ids = (self.content.author.pk,) + tuple(
            self.content.co_authors.all().values_list("pk", flat=True)
        )
...
class SubCompetence(TimeStampedModel):
    DEFAULT_NAMES = (
        ...
        (
            412,
            "4.1.2 Strategien zum Schutz (gegen z.B. Cyberkriminalität) entwickeln und anwenden",
        ),
        (42, "4.2 Persönliche Daten und Privatsphäre schützen"),
        (
            421,
            "4.2.1 Maßnahmen für Datensicherheit und gegen Datenmissbrauch kennen und beachten",
        ),
        (
            422,
        ...
        (
            551,
            "5.5.1 Funktionsweisen und grundlegende Prinzipien der digitalen Welt kennen, verstehen und bewusst nutzen",
        ),
        (
            552,
    ...
    slug = DllSlugField(max_length=512, populate_from="name")
